    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#f4f5f7] dark:bg-[#13161f]">
        <div className="text-center">
          <p className="text-[16px] font-semibold text-[#181d26] dark:text-[#e8eaf0] mb-2">{t('sharedBalance.snapshot.invalidTitle')}</p>
          <p className="text-[13px] text-[#9297a0]">{t('sharedBalance.snapshot.invalidBody')}</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="min-h-screen bg-[#f4f5f7] dark:bg-[#13161f]">
      ...
      <div className="bg-amber-50 dark:bg-amber-900/20 border-b border-amber-200 dark:border-amber-700 px-6 py-2 text-[12px] text-amber-700 dark:text-amber-300 text-center">
        {t('sharedBalance.snapshot.banner', { date: formatDate(snapshot.createdAt.slice(0, 10), language) })}
      </div>
          ...
          {[
            ...
            <div key={name} className={`${CARD} p-5`}>
              ...
              <div className="space-y-2 mb-4">
                {[
                  { key: 'totalPaid', label: t('sharedBalance.ledger.totalPaidShared'), value: formatMoney(paid, currency) },
                  { key: 'share', label: t('sharedBalance.ledger.share', { pct }), value: formatMoney(shouldPay, currency) },
                ].map(({ key, label, value }) => (
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead>
                  <tr>
                    {[
                      { key: 'date', label: t('expenses.table.date'), align: 'left' },
                      { key: 'description', label: t('expenses.table.description'), align: 'left' },
                      { key: 'category', label: t('expenses.table.category'), align: 'left' },
                      { key: 'paidBy', label: t('sharedBalance.table.paidBy'), align: 'left' },
                      { key: 'amount', label: t('expenses.table.amount'), align: 'right' },
                    ].map((h) => (
                      <th key={h.key} className={`text-[11px] font-semibold uppercase text-[#9297a0] border-b border-[#e8e8e8] dark:border-[#2d3347] py-2.5 px-4 ${h.align === 'right' ? 'text-right' : 'text-left'}`}>{h.label}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
            ...
            <div className={`${CARD} divide-y divide-[#e8e8e8] dark:divide-[#2d3347]`}>
              {sortByDateDesc(cashEntries).map((c, i) => (
                <div key={i} className="flex items-center justify-between px-5 py-3">
                  ...
                  <span className="text-[13px] font-semibold text-[#181d26] dark:text-[#e8eaf0] ml-2">{formatMoney(c.amount, currency)}</span>
                </div>
              ))}
            </div>
          </div>
